...
// Thin local adapter mirroring the minimum codex-tools schema/export path that
// we need for Responses API function tools, without importing the full
// codex-tools dependency graph.
...
pub fn tool_definition_to_responses_api_tool(tool_definition: ToolDefinition) -> ResponsesApiTool {
    ResponsesApiTool {
        ...
        parameters: strip_top_level_combinators(tool_definition.input_schema),
...
fn strip_top_level_combinators(mut schema: JsonSchema) -> JsonSchema {
    schema.any_of = None;
    ...
    if !matches!(
        ...
        schema.schema_type = Some(JsonSchemaType::Single(JsonSchemaPrimitiveType::Object));
    }
    if schema.properties.is_none() {
        schema.properties = Some(BTreeMap::new());
    }
    schema
}
